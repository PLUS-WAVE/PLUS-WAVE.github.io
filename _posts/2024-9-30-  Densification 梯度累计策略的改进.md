...
2. `densify_and_clone` 克隆:
   ...
   2. 直接复制一个添加即可（xyz不需要改变 —— 之后训练会优化）<!--more-->
...
如果计算出的梯度范数 $\begin{Vmatrix}\frac{dL}{d\mathbf{x}}\end{Vmatrix}_2$ 超过预设的阈值 $\tau_{\mathbf{x}}$，则该高斯函数会被选为致密化的候选对象。这意味着它可能处于一个需要增加更多点来更好描述的区域。 
